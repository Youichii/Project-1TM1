...
CREATE PROCEDURE proc_valeurs(in utilisateur varchar(16)) 
RESULT (idCom VARCHAR(16), nom VARCHAR(30), prenom VARCHAR(30), sexe CHAR(1), telephone VARCHAR(15), mail VARCHAR(40), anniversaire DATE, photo VARCHAR(50), portrait VARCHAR(120), 
        tire_tache VARCHAR(100), commentaire LONG VARCHAR, auteur VARCHAR(16), cote INTEGER, ville VARCHAR(50), nombreCote INTEGER)
BEGIN 
    Call sa_set_http_header('Access-Control-Allow-Origin', '*');
    select commu.idCom, nom, prenom, sexe, telephone, mail, anniversaire, photo, portrait, titre_tache, commentaire, auteur, cote, fct_informations(utilisateur), nombreCote
